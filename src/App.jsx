...
const sg = (w, o) => `stroke="${G}" stroke-width="${w}" opacity="${o}"`
...
const I18N = {
  RU: {
    ...
    counter:'0 / 3', btnReveal:'✦  Получить расклад',
    ...
    btnRestart:'Новый расклад', copied:'✓  Скопировано',
  ...
  KZ: {
    ...
    counter:'0 / 3', btnReveal:'✦  Ашылуды алу',
    ...
    btnRestart:'Жаңа ашылу', copied:'✓  Көшірілді',
  ...
  EN: {
    ...
    counter:'0 / 3', btnReveal:'✦  Get your reading',
    ...
    btnRestart:'New reading', copied:'✓  Copied',
...
function highlightCardNames(text) {
  ...
  names.forEach(name => {
    ...
    result = result.replace(re, `<span class="card-highlight">$1</span>`)
  })
  return result
}
...
function StreamText({ text, delay = 0, speed = 22 }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const interval = setInterval(() => {
        i++
        setDisplayed(highlighted.slice(0, i * 3))
        if (i * 3 >= highlighted.length) {
          setDisplayed(highlighted)
          setDone(true)
...
export default function App() {
  ...
  // Build shuffled deck
  function buildDeck() {
    setShuffledDeck([...ARCANA].sort(() => Math.random() - 0.5))
  }
  ...
  useEffect(() => {
    ...
    shuffleTimer.current = setInterval(() => {
      ...
      setShuffledDeck(prev => {
        const next = [...prev]
        // Swap 2 random unselected cards
  ...
  function toggleCard(idx) {
    if (selectedCards.includes(idx)) {
      setSelectedCards(prev => prev.filter(i => i !== idx))
    } else {
      if (selectedCards.length >= 3) return
  ...
  function copyText() {
    ...
    navigator.clipboard.writeText(text).then(() => showToast('✦  Текст скопирован в буфер обмена'))
      ...
      <div className="app">
          ...
          <div className="screen active">
              ...
              {[0,1,2].map(i => {
                ...
                return (
                  <div key={i} className={`preview-slot${c?' filled':''}`}>
                    {c
                      ? <><span className="slot-emoji" style={{width:28,height:28,display:'flex'}} dangerouslySetInnerHTML={{__html:c.svg}}/><span className="slot-pos">{t.positions[i]}</span></>
            ...
            <div className="cards-grid">
              {shuffledDeck.map(card => {
                const isSelected = selectedCards.includes(card.n)
                const isDimmed = !isSelected && selectedCards.length === 3
                return (
                  <div key={card.n}
                    className={`card-tile${isSelected?' selected':''}${isDimmed?' dimmed':''}`}
                    onClick={() => !isDimmed && toggleCard(card.n)}>
        ...
        {screen === 'result' && result && (
          <div className="screen active">
            ...
            <div className="result-strip">
              {resultCards.map((c,i) => (
                <div key={i} className="result-card" style={{animationDelay:`${i*0.15+0.1}s`}}>
                  <div className="rc-sym" style={{display:'flex',alignItems:'center',justifyContent:'center',width:48,height:48,margin:'0 auto 8px'}} dangerouslySetInnerHTML={{__html:c.svg}}/>
            ...
            <div style={{width:'100%'}}>
              {[
                {label: t.blockLabels.grid,  text: result.grid,   delay: 300},
                {label: t.blockLabels.blind, text: result.blind,  delay: 1200},
                {label: t.blockLabels.action,text: result.action, delay: 2400},
            ...
            {result.shadow && (
              <div className="shadow-block">
                ...
                <div className="shadow-question">
                  <StreamText text={result.shadow} delay={3800} speed={60}/>
                </div>
              </div>
            )}
            ...
            <div className="share-row">
              <button className="btn-share-story" onClick={() => showToast('✦  Функция шеринга — в полной версии')}>
                <span>✦</span><span>{t.btnStory}</span>
              </button>
              <button className="btn-share-copy" onClick={copyText}>
