...
const I18N = {
  RU: {
    ...
    btnShareImprint:'Поделиться слепком', btnCopyImprint:'Скопировать',
  ...
  KZ: {
    ...
    btnShareImprint:'Ізді бөлісу', btnCopyImprint:'Көшіру',
  ...
  EN: {
    ...
    btnShareImprint:'Share imprint', btnCopyImprint:'Copy',
...
function highlightCardNames(text) {
  ...
  let result = text
  names.forEach(name => {
    result = result.replace(new RegExp(`(${name})`, 'gi'), `<span class="card-highlight">$1</span>`)
  })
  return result
}
...
function parseResponse(raw) {
  const ex = (tag, next) => {
    const m = raw.match(new RegExp(`##${tag}##\\s*([\\s\\S]*?)(?=##${next}##|$)`, 'i'))
    return m ? m[1].trim() : ''
  }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const interval = setInterval(() => {
        i += 3
        if (i >= highlighted.length) { setDisplayed(highlighted); clearInterval(interval); return }
        setDisplayed(highlighted.slice(0, i))
      }, speed)
...
export default function App() {
  ...
  const stars = Array.from({length:60},(_,i)=>({
    ...
    size:Math.random()<0.12?2:1, dur:2+Math.random()*4,
    delay:Math.random()*5, minOp:0.05+Math.random()*0.08, maxOp:0.25+Math.random()*0.45
  }))
  ...
  async function revealCards() {
    ...
    try {
      const res = await fetch('/api/oracle', {
        ...
        body: JSON.stringify({ question, cards: cardStr, lang })
      })
      clearInterval(lt)
  ...
  async function callShadow(history, iteration, cardIdx, isOpening=false) {
    ...
    try {
      const res = await fetch('/api/shadow', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({
          cardIndex: cardIdx ?? shadowDominantIdx,
          history: isOpening ? [] : history,
          shadowQuestion: shadowQ,
          cards, lang,
          iteration,
          maxIterations: SHADOW_MAX
        })
      })
      const data = await res.json()
      ...
      const newMsg = { role:'assistant', content: cleanReply, voiceName: vname }
      setShadowMessages(prev => [...prev, newMsg])
      ...
      if (isFinal || iteration >= SHADOW_MAX - 1) {
        setShadowDone(true)
        setTimeout(() => generateImprint(newHistory, vname), 1500)
      }
    } catch(e) {
      setShadowTyping(false)
      showToast('Ошибка соединения с Тенью')
    }
  }
  ...
  async function generateImprint(history, voiceName) {
    showToast('Тень формирует твой слепок...')
    try {
      const res = await fetch('/api/imprint', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ history, voiceName, lang })
      })
      const data = await res.json()
      ...
      const imp = {
        archetype: voiceName,
        resource: ex('RESOURCE','BLOCK'),
        block: ex('BLOCK','GROWTH'),
        growth: ex('GROWTH','XXXXEND')
      }
      setImprint(imp)
      setScreen('imprint')
  ...
  function shareResult(text, title) {
    if (navigator.share) {
      navigator.share({ title, text }).catch(() => copyToClipboard(text))
  ...
  function restart() {
    ...
    setShadowHistory([]); setShadowIteration(0); setImprint(null)
  }

  const resultText = result ? [
    `THE ORACLE — ${resultCards.map(c=>c.name).join(' · ')}`,
    '', result.grid, '', result.blind, '', result.action, '', `✦ ${result.shadow}`
  ].join('\n') : ''

  const imprintText = imprint ? [
    `ЦИФРОВОЙ СЛЕПОК · ${imprint.archetype}`,
    '', imprint.resource, '', imprint.block, '', `✦ ${imprint.growth}`
  ].join('\n') : ''
  ...
  return (
    <div className="app-root">
      <div className="starfield">
        {stars.map(s=>(
          <div key={s.id} className="star" style={{top:`${s.top}%`,left:`${s.left}%`,width:s.size,height:s.size,'--d':`${s.dur}s`,'--dl':`${s.delay}s`,'--lo':s.minOp,'--hi':s.maxOp}}/>
        ))}
      </div>
      ...
      <div className="app">
        ...
        {screen==='cards' && (
          <div className="screen active">
            ...
            <div className="preview-row">
              {[0,1,2].map(i=>{
                const c = selectedCards[i]!==undefined ? ARCANA[selectedCards[i]] : null
                return (
                  <div key={i} className={`preview-slot${c?' filled':''}`}>
                    {c ? <><span style={{width:28,height:28,display:'flex'}} dangerouslySetInnerHTML={{__html:c.svg}}/><span className="slot-pos">{t.positions[i]}</span></>
                       : <span style={{fontSize:13,color:'var(--text-muted)'}}>{['I','II','III'][i]}</span>}
                  </div>
                )
              })}
            </div>
            <div className="cards-grid">
              {shuffledDeck.map(card=>{
                const isSel = selectedCards.includes(card.n)
                const isDim = !isSel && selectedCards.length===3
                return (
                  <div key={card.n} className={`card-tile${isSel?' selected':''}${isDim?' dimmed':''}`} onClick={()=>!isDim&&toggleCard(card.n)}>
                    <div className="card-inner">
                      <div className="card-back-face">
                        <div className="back-corner tl"/><div className="back-corner tr"/>
                        <div className="back-corner bl"/><div className="back-corner br"/>
                        <div className="back-sigil" dangerouslySetInnerHTML={{__html:BACK_SVG}}/>
                      </div>
                      <div className="card-front-face">
                        <div className="front-corner tl"/><div className="front-corner tr"/>
                        <div className="front-corner bl"/><div className="front-corner br"/>
            ...
            <button className="btn-primary" style={{marginTop:24}} onClick={revealCards} disabled={selectedCards.length!==3}>
              <span>{t.btnReveal}</span>
            </button>
          </div>
        )}
        ...
        {screen==='result' && result && (
          <div className="screen active">
            ...
            <div style={{width:'100%'}}>
              {[{label:t.blockLabels.grid,text:result.grid,delay:300},{label:t.blockLabels.blind,text:result.blind,delay:1200},{label:t.blockLabels.action,text:result.action,delay:2400}]
            ...
            <div className="share-row">
              <button className="btn-share-story" onClick={()=>shareResult(resultText,'THE ORACLE · Мой расклад')}>
                <span>✦</span><span>{t.btnStory}</span>
              </button>
              <button className="btn-share-copy" onClick={()=>copyToClipboard(resultText)}>
                <span>⎘</span><span>{t.btnCopy}</span>
              </button>
            </div>
            <button className="btn-shadow-cta" onClick={startShadowAudience}>
              <span className="btn-shadow-icon">🌑</span>
              <div className="btn-shadow-content">
                <span className="btn-shadow-title">{t.btnShadowCta}</span>
                <span className="btn-shadow-sub">{t.btnShadowSub}</span>
              </div>
        ...
        {screen==='shadow' && (
          <div className="screen active">
            <div className="shadow-session-header">
              {SIGIL(40)}
              <div>
                <div className="shadow-session-title">Аудиенция с Тенью</div>
                <div className="shadow-session-sub">{shadowVoiceName}</div>
              </div>
              <div className="shadow-counter">{Math.min(shadowIteration,SHADOW_MAX)}/{SHADOW_MAX}</div>
            </div>
            <div className="shadow-progress">
              <div className="shadow-progress-bar" style={{width:`${Math.min((shadowIteration/SHADOW_MAX)*100,100)}%`}}/>
            </div>
            <div className="shadow-chat" ref={chatRef}>
              ...
              {shadowTyping && (
                <div className="shadow-typing"><span/><span/><span/></div>
              )}
            </div>
            {!shadowDone && (
              <div className="shadow-input-area">
                <textarea
                  ...
                  onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendToShadow()}}}
                  placeholder="Твой ответ Тени..."
                  disabled={shadowTyping}
                />
                <button className="shadow-send-btn" onClick={sendToShadow} disabled={shadowTyping||!shadowInput.trim()}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                    <path d="M5 12H19M19 12L13 6M19 12L13 18" stroke="#c9a84c" strokeWidth="1.5" strokeLinecap="round"/>
                  </svg>
                </button>
              </div>
            )}
        ...
        {screen==='imprint' && imprint && (
          <div className="screen active">
            ...
            <div className="share-row" style={{animationDelay:'0.9s'}}>
              <button className="btn-share-story" onClick={()=>shareResult(imprintText,'Мой Цифровой Слепок Тени')}>
                <span>✦</span><span>{t.btnShareImprint}</span>
              </button>
              <button className="btn-share-copy" onClick={()=>copyToClipboard(imprintText)}>
                <span>⎘</span><span>{t.btnCopyImprint}</span>
              </button>
            </div>
